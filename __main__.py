...
def main(argv):
    ...
    # A UDP traceroute routine for Linux and OSX  - must be run as root
    # destAddr can also be a domain name (it will do a name lookup)
    # On the first attempt it will send to destPort. if it gets no response, it will then alternate between
    # sending udp packets to the fallbackPort and destPort
    def tracerouteLinuxOSX(ipAddrofSendingInterface, destHost, destPort, fallbackPort=33434, noOfRetries=6, timeOut=0.5, \
                          maxNoOfHops=16):
        ...
        # Create tx (udp) and rx (icmp) sockets
        udpTx, icmpRx = createSockets(ipAddrofSendingInterface)

        print('traceroute to ' + str(destHost) + "/" + str(destIPAddr))
        # Perform the traceroute
        # This loop counts the hops
        while ttl < maxNoOfHops:
            attemptsCount = 1
            ttl += 1
            ...
            while attemptsCount < noOfRetries:
                ...
                sendUDP(udpTx, ttl, b'isptest', destIPAddr, udpTxPort)
